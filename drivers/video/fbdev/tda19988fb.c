 ...
 */
int lcd_fb_mode_is_equal(const struct fb_videomode *mode1, const struct fb_videomode *mode2) {
	return (mode1->xres == mode2->xres && mode1->yres == mode2->yres && mode1->hsync_len == mode2->hsync_len && mode1->vsync_len == mode2->vsync_len && mode1->left_margin == mode2->left_margin && mode1->right_margin == mode2->right_margin && mode1->upper_margin == mode2->upper_margin && mode1->lower_margin == mode2->lower_margin &&
			/* refresh check, 59.94Hz and 60Hz have the same parameter in struct of cea_mode */
...
static uint8_t cec_read(struct tda19988_data *priv, uint8_t addr) {
...
static void reg_write(struct tda19988_data *data, uint16_t reg, uint8_t val) {
	...
	int ret;
	printk(KERN_ERR
	"TDA: %s writing 0x%08x to 0x%08x\n", __FUNCTION__, val, reg);
...
static void reg_set(struct tda19988_data *data, uint16_t reg, uint8_t val) {
	int old_val;
	printk(KERN_ERR
	"TDA: %s setting 0x%08x to 0x%08x\n", __FUNCTION__, reg, val);
...
static void reg_clear(struct tda19988_data *data, uint16_t reg, uint8_t val) {
	...
	printk(KERN_ERR
	"TDA: %s clearing 0x%08x from 0x%08x \n", __FUNCTION__, val, reg);
	old_val = reg_read(data, reg);
	if(old_val >= 0) {
		reg_write(data, reg, old_val & ~val);
	}
...
static void tda19988_write_if(struct tda19988_data *data, uint8_t bit, uint16_t addr, uint8_t *buf, size_t size) {
	printk(KERN_ERR
	"TDA: %s\n", __FUNCTION__);
...
static void tda19988_configure_audio(struct tda19988_data *data, struct fb_var_screeninfo var) {
	...
	buf[3] = IEC958_AES4_CON_ORIGFS_NOTID | IEC958_AES4_CON_MAX_WORDLEN_24;
...
static void tda998x_hpd(struct work_struct *work) {
	...
	struct tda19988_data *priv = container_of(det_work,	struct tda19988_data, det_work);
	printk(KERN_ERR
	"TDA: %s\n", __FUNCTION__);
...
static irqreturn_t tda998x_irq_thread(int irq, void *data) {
	...
	dev_err(&priv->cec->dev, "tda irq sta %02x cec %02x lvl %02x f0 %02x f1 %02x f2 %02x\n", sta, cec, lvl, flag0, flag1, flag2);
...
static void tda19988_fb_var(struct tda19988_data *data, struct fb_var_screeninfo var) {
	...
	reg_write(data, REG_HVF_CNTRL_0, HVF_CNTRL_0_PREFIL(0) | HVF_CNTRL_0_INTPOL(0));
	reg_write(data, REG_VIP_CNTRL_5, VIP_CNTRL_5_SP_CNT(0));
	reg_write(data, REG_VIP_CNTRL_4, VIP_CNTRL_4_BLANKIT(0) | VIP_CNTRL_4_BLC(0));

	reg_clear(data, REG_PLL_SERIAL_1, PLL_SERIAL_1_SRL_MAN_IZ);
	reg_clear(data, REG_PLL_SERIAL_3, PLL_SERIAL_3_SRL_CCIR | PLL_SERIAL_3_SRL_DE);
	...
	reg_write(data, REG_SEL_CLK, SEL_CLK_SEL_VRF_CLK(0) | SEL_CLK_SEL_CLK1 | SEL_CLK_ENA_SC_CLK);

	reg_write(data, REG_PLL_SERIAL_2, PLL_SERIAL_2_SRL_NOSC(div) | PLL_SERIAL_2_SRL_PR(rep));

	/* set color matrix bypass flag: */
	reg_write(data, REG_MAT_CONTRL, MAT_CONTRL_MAT_BP | MAT_CONTRL_MAT_SC(1));
	...
	dev_info(&data->cec->dev, "Matching CEA mode is %d for fb mode %dx%d-%d", cea_mode, var.xres, var.yres, mode.refresh);
...
static int tda19988_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	cec_write(data, REG_CEC_FRO_IM_CLK_CTRL, CEC_FRO_IM_CLK_CTRL_GHOST_DIS | CEC_FRO_IM_CLK_CTRL_IMCLK_SEL);
	...
	if(client->irq) {
		...
		int ret = request_threaded_irq(client->irq, NULL, tda998x_irq_thread, irqf_trigger | IRQF_ONESHOT, "tda998x", data);
		if(ret) {
			printk(KERN_ERR
			"TDA: Failed requesting IRQ %d (%d)\n", client->irq, ret);
...
static int read_edid_block(void *data, u8 *buf, unsigned int blk, size_t length) {
	...
	if(priv->hdmi->irq) {
		printk(KERN_ERR
		"TDA: edid read waiting on IRQ %d\n", priv->hdmi->irq);
